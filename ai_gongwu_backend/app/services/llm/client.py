"""
这个文件封装旧后端的 LLM 调用；提示词和网络请求分开写，是为了回归时能替换真实模型或测试桩。
...
class LLMClient:
    """
    大模型客户端。 这里使用“保守策略”： - temperature 低一点，减少发散 - 尽量要求 JSON-only 输出 - 失败时自动重试
    ...
    def generate(
        ...
        """
        生成结构化评分结果。 这里的完整策略是： 1. 先按严格 JSON 模式请求 2. 如果模型输出不合法，保留原输出 3. 下一轮把“上一轮不合法”告诉模型，让它自我修复 4. 多次失败后返回 None，由上层决定怎么报错
