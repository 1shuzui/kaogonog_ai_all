/**
 * 媒体录制组合函数封装浏览器 MediaRecorder 差异，保证考场页能在音频和视频模式之间复用同一套状态。
 *
 * 它只产出本地录制 Blob 和时长，不参与 ASR、VAD、评分或权益扣减；提交后的识别和评分必须留给后端管线处理。
 *
 * @param 无；导出的 useMediaRecorder 在运行时接收外部媒体流或初始化选项。
 * @return 导出媒体流、录制状态、错误信息、时长和初始化/开始/停止等操作。
