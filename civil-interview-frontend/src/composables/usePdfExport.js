/**
 * PDF 导出组合函数把结果页或复盘资料渲染成离线报告，方便用户保存训练记录。
 *
 * 导出发生在评分完成之后，只读取当前 DOM，不重新计算分数或改写历史；动态加载依赖是为了不让普通页面承担 PDF 相关体积。
 *
 * @param 无；导出的 exportToPdf 接收待导出的 DOM 节点和文件名。
 * @return 导出导出中状态和 PDF 生成操作。
