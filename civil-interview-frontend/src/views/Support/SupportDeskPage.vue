<!--
客服反馈管理页，给管理员查看用户反馈、处理状态和回复记录。

反馈处理只能更新状态和管理员备注，不能改用户原始描述；附件、页面路径和题目上下文用于定位问题来源。

@param: 无；筛选条件来自管理员输入，反馈列表和处理结果来自 support API。
@return: 渲染反馈列表、筛选器、处理弹窗和附件预览入口。
@raises: 不主动抛业务异常；非管理员、附件读取失败或处理失败由页面提示承接。
-->
<template>
