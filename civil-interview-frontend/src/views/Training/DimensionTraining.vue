<!--
专项题型训练页，围绕单一题型生成和练习，帮助用户集中突破具体题型。

本页只按题型生成训练题，不负责评分能力分析；方向、地区和题型筛选最终都交给后端题库匹配。

@param: 无；题型 key 来自路由，题目来自 training API。
@return: 渲染题型说明、生成结果、练习入口和空态。
@raises: 不主动抛业务异常；题型无效、题库无匹配或权益不足由页面提示承接。
-->
<template>
