<!--
退款管理页，给管理员核对微信虚拟支付订单、查询退款和处理异常支付单。

退款会影响订单和权益两条链路，本页只给管理员发起核查和处理请求，不在前端直接改用户余额。

@param: 无；筛选条件来自管理员输入，订单和退款结果来自 payment 管理接口。
@return: 渲染退款查询表单、订单摘要、可退余额和处理结果。
@raises: 不主动抛业务异常；非管理员、订单不存在或微信核验失败由请求层提示承接。
-->
<template>
