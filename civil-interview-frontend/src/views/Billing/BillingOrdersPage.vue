<!--
订单中心页，展示用户订单、支付状态和退款进度，帮助用户核对权益到账情况。

订单状态只是支付与到账记录的展示，权益余额仍以订阅接口返回为准；页面不能根据订单状态自行发放或扣减权益。

@param: 无；订单数据来自 billing/payment 接口和当前登录用户。
@return: 渲染订单列表、支付状态、退款状态和返回入口。
@raises: 不主动抛业务异常；未登录、订单查询失败或状态异常由请求层和页面提示承接。
-->
<template>
