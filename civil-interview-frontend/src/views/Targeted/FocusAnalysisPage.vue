<!--
重点分析页，展示真实题库统计或管理员发布内容；无数据时保持空态，不生成伪重点。

能力重点和高频题型分开展示：能力维度用于备考建议，题型分类用于训练入口；雷达图不稳定时保持隐藏。

@param: 无；分析条件来自路由 query 和 targeted store。
@return: 渲染重点分析、题型统计、策略建议、空态和生成练习入口。
@raises: 不主动抛业务异常；无题库数据、未登录或接口失败由页面空态提示承接。
-->
<template>
