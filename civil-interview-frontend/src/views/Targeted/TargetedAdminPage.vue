<!--
定向入口管理页，给管理员维护分类树、分析内容和发布状态，使用与用户端一致的上下层级。

管理员可以编辑发布后的重点分析内容和数值，但分类树本身仍以后端真实题源口径为准；停用后回到自动统计或空态。

@param: 无；分类选择和表单内容来自 targeted API 与管理员输入。
@return: 渲染上下式分类选择、分析编辑表单、发布/停用和重新分析入口。
@raises: 不主动抛业务异常；非管理员、保存失败或无题库数据由页面提示承接。
-->
<template>
