<!--
管理员权益调整流水页，用于按用户、操作者、类型和时间筛选补发/扣减记录，重点保证人工调整可追溯。

流水只读展示，不提供删除或真正撤销；误操作需要通过反向调整纠正，保证客服处理历史完整。

@param: 无；筛选条件来自管理员输入，列表来自 entitlementAdmin API。
@return: 渲染筛选表单、调整流水、改前改后摘要和分页。
@raises: 不主动抛业务异常；非管理员、查询失败或分页异常由请求层和页面提示承接。
-->
<template>
