<!--
PC 管理员用户权益页，负责按用户名查询用户、查看订单与权益摘要、补发人工权益和扣减指定权益。

这里的表单不是支付后台：补发不会创建微信订单，扣减不会写答题用量，所有动作都要求原因和备注并写入审计流水。
前端的大额二次确认只是降低误操作概率，后端仍会校验管理员身份、剩余分钟和参数边界。

@param: 无；页面通过搜索框、弹窗表单和路由状态驱动请求。
@return: 渲染用户权益详情、调整弹窗、最近流水和操作结果提示。
@raises: 不主动抛业务异常；接口返回 401/403/400 时由请求层和页面提示承接。
-->
<template>
