<!--
江苏事业单位岗位页，按岗位方向展示题库入口，用户可见文案不再展示 ABCDE 类。

内部路由 key 仍保留旧代码以兼容跳转，但用户只看到岗位名称；页面不能把 ABCDE 类重新拼回标题或题目说明。

@param: 无；岗位 key 来自路由，展示文案来自 jiangsuJobs 配置。
@return: 渲染岗位介绍、题库入口、年份/题型筛选和开始练习入口。
@raises: 不主动抛业务异常；岗位 key 无效或题库无匹配由页面空态承接。
-->
<template>
