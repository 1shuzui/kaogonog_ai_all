<!--
PC 题库列表页，负责按考试体系、地区、年份、岗位和题型筛选真实题库，并提供管理员编辑入口。

这个页面展示题库检索结果，不负责重新判断题目真实分类；分类纠偏应在导入脚本、后端服务和管理员编辑里完成。
用户侧筛选要尊重真实题源，不能把江苏事业单位、安徽省考、湖南省考等来源串到别的分类。

@param: 无；筛选状态来自页面表单、路由参数和 questionBank store。
@return: 渲染题目列表、筛选器、空态、分页和管理员操作入口。
@raises: 不主动抛业务异常；查询失败或无权限由请求层和页面提示承接。
-->
<template>
