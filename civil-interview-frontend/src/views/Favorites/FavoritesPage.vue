<!--
收藏与错题页，集中展示用户保存的题目和错题入口，状态互不强行合并。

收藏和错题来源不同：收藏来自用户主动保存，错题来自评分结果；页面只展示两类入口，不把二者互相覆盖。

@param: 无；列表数据来自 favorites store 和用户切换的 tab。
@return: 渲染错题/收藏列表、空态和重新练习入口。
@raises: 不主动抛业务异常；未登录、题目缺失或接口失败由请求层和页面提示承接。
-->
<template>
