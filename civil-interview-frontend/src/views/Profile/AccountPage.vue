<!--
账户安全页，处理密码、资料和账号设置，避免个人信息散落在多个页面。

资料更新只影响展示和默认偏好，不能在这里修改管理员身份、权益或微信绑定结果；敏感校验交给后端。

@param: 无；表单初值来自 user store，保存动作调用用户资料和密码接口。
@return: 渲染资料、密码、安全提示和保存反馈。
@raises: 不主动抛业务异常；未登录、旧密码错误或资料保存失败由页面提示承接。
-->
<template>
