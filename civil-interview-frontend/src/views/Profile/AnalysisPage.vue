<!--
能力分析页，展示考生答题能力维度和薄弱项；这里不能混用题型训练分类。

能力维度来自评分结果和历史记录，题型训练分类只用于训练入口；数据不足时保持空态，不用题型硬凑能力图。

@param: 无；分析数据来自 history/user 相关接口和当前用户。
@return: 渲染能力概览、薄弱项、趋势和练习建议。
@raises: 不主动抛业务异常；未登录、历史不足或接口失败由页面空态承接。
-->
<template>
