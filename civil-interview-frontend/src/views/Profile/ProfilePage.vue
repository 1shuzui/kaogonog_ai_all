<!--
个人中心页，展示用户资料、权益、历史入口和管理员工作台入口。

个人中心只汇总入口和快照：权益余额来自订阅接口，管理员入口来自后端身份；页面不自行判断后台权限。

@param: 无；用户、权益和历史摘要来自 user/billing/history store。
@return: 渲染用户卡片、权益摘要、常用入口、客服入口和管理员工作台入口。
@raises: 不主动抛业务异常；未登录或数据刷新失败由页面空态和提示承接。
-->
<template>
