/**
 * PC 江苏事业单位岗位入口保留历史路由 key 和内部 code，但用户可见文案统一改成岗位方向。
 *
 * 这样既不破坏旧链接和筛选参数，又避免把 ABCDE 类误展示成当前产品分类；这些入口只服务江苏事业单位统考，不扩展到江苏省考。
 *
 * @param 无；导出函数接收岗位 key、定向 code 或筛选条件。
 * @return 导出江苏岗位、地市、年份、题型常量和本地演示题目构造函数。
