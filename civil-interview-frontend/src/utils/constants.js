/**
 * PC 常量集中区刻意把评分能力维度、训练题型分类、地区和套餐展示枚举放在一起，减少页面各写一套口径。
 *
 * 这里最重要的边界是“能力维度”和“题型分类”不能混用：行政思维等用于评分，组织管理等用于训练和题库筛选。
 *
 * @param 无；常量模块不读取运行时业务数据。
 * @return 导出端侧枚举、文案映射和轻量查找函数，供 PC 页面复用。
