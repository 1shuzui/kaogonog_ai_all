/**
 * PC 定向备面兜底分类树用于接口异常或旧缓存时维持真实考试体系层级，而不是展示泛化模板。
 *
 * 这里的树同时服务用户端和管理员端：法检等特殊体系可以声明动态层级名，方向允许“不限”，并列地区或银行类型必须拆成同级选项。
 * 兜底树只保证入口可浏览；重点分析和出题仍以后端真实题库统计或管理员发布内容为准。
 *
 * @param 无；导出函数接收定向节点或目标载荷。
 * @return 导出兜底分类树、节点构造函数和目标参数归一函数。
