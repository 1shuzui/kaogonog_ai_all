/**
 * PC 评分辅助工具先判断题目是否接入评分题库，再把后端错误转成用户能理解的提示。
 *
 * 本地生成、草稿或临时题不能直接送外部评分，否则会出现“题目不存在”一类误导错误；能力维度展示仍以后端评分响应为准。
 *
 * @param 无；导出函数接收题目 id、题目对象或原始错误消息。
 * @return 导出评分支持判断、不可评分文案和错误消息归一函数。
