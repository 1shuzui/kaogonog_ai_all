/**
 * PC 账单状态仓库保存套餐展示、付费墙和本地订单摘要，真正的权益余额仍以后端订阅接口为准。
 *
 * 这个 store 不能自行补发时长或确认支付成功；微信小程序虚拟支付、管理员人工调整和退款都必须走后端审计链路。
 * 本地持久化只用于刷新页面后的展示连续性，任何付费模块进入前都要重新校验服务端权益。
 *
 * @param 无；actions 接收套餐选择、付费墙来源和训练会话状态。
 * @return 导出 Pinia store，供套餐中心、订单页、付费墙和权益守卫复用。
