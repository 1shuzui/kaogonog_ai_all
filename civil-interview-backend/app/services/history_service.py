...
def _normalize_dimension_name(name: str) -> str:
    return "行政思维" if name == "法治思维" else name
...
def _normalize_dimensions(dimensions) -> list:
    ...
    for item in dimensions:
        if isinstance(item, dict):
            normalized.append({**item, "name": _normalize_dimension_name(str(item.get("name") or ""))})
    return normalized


def _normalize_score_result(score_result):
    if not isinstance(score_result, dict):
        return score_result or {}
    if not isinstance(score_result.get("dimensions"), list):
        return score_result
    return {**score_result, "dimensions": _normalize_dimensions(score_result.get("dimensions"))}
...
def get_history_stats(db: Session, username: str) -> dict:
    ...
    for r in rows:
        for dim in _normalize_dimensions(r.dimensions):
            name = dim.get("name")
            if name in totals:
