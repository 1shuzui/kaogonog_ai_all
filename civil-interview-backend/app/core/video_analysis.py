...
面试评分可以参考入镜稳定性、人物距离和头部晃动，但这些观察不能替代内容评分，也不能因为 OpenCV 或视频文件异常让整次评分失败。因此本文件只做低成本抽帧和人脸检测，把结果转成中文观察文本交给评分上下文；识别不到、文件缺失或运行环境没有 OpenCV 时都返回可解释提示。

@param: 模块本身无入参；业务输入来自评分服务传入的本地视频路径。
@return: 导出 analyze_video_behavior，返回一段用于辅助评分的中文观察文本。
@raises ImportError: 基础路径依赖缺失时会在导入阶段失败；OpenCV 缺失会在函数内降级为提示文本。
"""
