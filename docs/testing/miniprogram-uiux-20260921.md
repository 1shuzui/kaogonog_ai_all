...
按用户进一步要求，将“全真练习 / 套餐中心”整组移动到首页主练习入口之后、近期练习和游客浏览提示之前。游客与已登录状态都使用同一位置，不重复入口；样式、事件处理、全真模式参数、套餐/登录权限保持不变。新增真实Vue渲染顺序回归先失败再通过，首页/登录专项12项通过，微信生产构建与文档检查通过。本次仅调整首页顺序，不更新后端或PC。
