...
小程序在当前定向页原位展示分析，不用全屏遮罩锁住导航。客户端以任务编号和完整参数快照隔离迟到响应，同参数失败保留上次有效结果；10 秒仅补充等待提示，30 秒按接口约定超时。用户可以“停止等待”，这只中断客户端连接，不承诺取消服务端执行。原重点分析深链继续可用，其参数与请求共用字段表，不能遗漏城市或计时字段。响应结构异常进入可重试错误态，不当成空报告；分类树请求失败只用临时兜底，下次进入允许重试。实现与验证见 [交互修复记录](../testing/miniprogram-interaction-20260920.md)。
