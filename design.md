...
首页先复盘/练习；准备先模式/录制/真实摘要，常驻底部开始入口；题库常用条件前置、更多条件折叠；结果先真实分数与已有建议，完整原文可展开；微信登录主入口、PC已有账号次入口。没有新增虚构学习数据或一题默认规则，默认仍5题。保留既有280ms实测高度折叠，不以 scaleY 代替布局过渡；reduced/off 立即收敛。完整验证与限制见 [UI/UX 修复记录](docs/testing/miniprogram-uiux-20260921.md)。
