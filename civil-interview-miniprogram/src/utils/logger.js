/**
 * 小程序日志工具统一控制输出级别并脱敏 token、openid、session_key 和支付签名等敏感字段。
 *
 * 移动端支付、登录和录音问题需要足够排查信息，但不能把微信身份凭证或虚拟支付参数打印到开发工具和线上日志里。
 *
 * @param 无；导出函数接收日志级别、消息和结构化 metadata。
 * @return 导出脱敏、requestId 生成和分级日志方法，供 API 层、store 和页面复用。
