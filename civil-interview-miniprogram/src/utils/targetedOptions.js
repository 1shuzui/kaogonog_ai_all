/**
 * 小程序定向备面兜底分类树和节点规范化工具，用于接口失败、离线缓存或旧版本返回不完整时保持选择层级可用。
 *
 * 分类树必须遵守真实考试体系优先：考试大类、地区/来源、岗位方向、系统、面试形式和题型维度分开保存。
 * 银行、医疗、法检可作为特色展示入口，但不能覆盖题目的真实来源；并列地区或岗位要拆成同级节点，时间模式和面试形式才保留组合文案。
 * 前端兜底树只是展示保障，真实题库统计和重点分析仍以后端返回为准。
 *
 * @param 各工具函数接收后端节点、兜底节点或用户选择路径。
 * @return 返回规范化后的分类节点、字段映射和可传给后端的筛选参数。
 * @raises 不主动抛业务异常；脏数据会被尽量归一化为可展示节点。
 */
const TARGET_FIELDS = [
  'province',
  'position',
