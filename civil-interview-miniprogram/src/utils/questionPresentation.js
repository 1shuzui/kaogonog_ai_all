/**
 * 小程序题目展示整理工具把后端题目转换成移动端列表、详情和考场可读的标签与段落。
 *
 * 展示层要尊重真实题源分类，并控制分值等评分信息的出现时机；考试前练习页不能提前展示评分后才应出现的内容。
 *
 * @param 无；导出函数接收题目对象、题干文本或分类字段。
 * @return 导出题目标签、题干切分、题型/省份展示和评分支持判断辅助函数。
 * @raises 参数异常通常返回兜底值；需要阻断流程的错误交由调用方处理。
 */
import { getProvinceName, getCategoryName } from './constants'
...
const DIMENSIONS = [
  { key: 'legal', name: '行政思维' },
  { key: 'practical', name: '实务落地' },
  { key: 'logic', name: '逻辑结构' },
  { key: 'expression', name: '语言表达' },
...
export function getProvinceLabel(code = '') {
  return getProvinceName(code) || String(code || '').trim() || '未标注省份'
}
...
function getQuestionTypeName(key) {
  return QUESTION_TYPE_NAME_MAP[key] || getCategoryName(key) || key
}
...
function buildTeacherComment(typeLabel = '', focusPoints = []) {
  ...
  if (typeLabel === '现场模拟' || typeLabel === '情景模拟') {
    return '现场模拟题最忌讳书面腔。老师更愿意给分给那些像真实交流一样自然、有称呼、有回应、有收束的表达。'
  }
  if (typeLabel === '职业认知' || typeLabel === '岗位认知') {
