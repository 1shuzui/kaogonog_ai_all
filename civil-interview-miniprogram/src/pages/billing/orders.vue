<!--
小程序订单中心用于让用户核对虚拟支付订单与权益到账结果，展示口径必须跟服务端订单和微信官方虚拟支付状态一致。
页面只做查询和展示，不在端侧补发权益，也不把未确认订单当作已支付，避免破坏微信审核和售后账务链路。

@param: 无；页面在展示时读取登录态，并通过订单接口刷新当前用户订单。
@return: 渲染订单列表、选中订单详情和刷新入口。
