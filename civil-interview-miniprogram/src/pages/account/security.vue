<!--
小程序账户安全页把改密、微信绑定和 PC 登录账号设置集中到用户主动进入的场景，避免“我的”页直接暴露敏感操作。
账号同步依赖同一后端用户实体，页面不能在本地自行合并账号或绕过协议状态校验。

@param: 无；页面读取用户资料、协议状态和表单输入。
@return: 渲染密码修改、微信绑定、PC 账号设置和协议状态确认入口。
