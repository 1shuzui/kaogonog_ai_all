<!--
小程序专项题型训练页围绕单一训练分类生成题目，定向筛选只收窄题源，不改变评分能力维度。
“考试大类、地区、方向”允许保持不限，以便用户先按题型练习，再逐步加上真实考试体系约束。

@param: 无；页面从路由 key 读取训练分类，并结合定向树生成可选筛选项。
@return: 渲染题型说明、训练进度、定向筛选和生成后的题目列表。
