<!--
小程序答题考场页，串联题本展示、计时、录音/录像、麦克风权限确认、提交和跳转结果页。

全真模拟和专项练习都走这里，但表现不同：全真模拟保留题本区和真实套题进度，普通练习更轻量。
作答阶段只展示题干、题号、时间和媒体控制，不显示题目分值或评分后才应该出现的能力反馈；录音开始前先确认麦克风权限，避免用户讲完才发现没有录上。

@param: 无；考试状态来自 exam store，媒体能力来自 uni 录音/摄像 API。
@return: 渲染考场界面并在提交后进入评分/结果流程。
@raises: 不主动抛业务异常；权限拒绝、录音失败、上传失败和评分失败由页面提示承接。
-->
<template>
  <view
    class="exam-room"
